...
LineRenderer::LineRenderer(std::vector<Vector3> p_points, unsigned int p_vertexShader, unsigned int p_fragmentShader, Vector4 p_color) 
	: m_color(p_color)
{
	addPoints(p_points);
	setShaders(p_vertexShader, p_fragmentShader); 
...
void LineRenderer::addPoint(Vector3 p_point)
{
	m_points.push_back(p_point);
	//TODO : I need to make the shape of the line smoother
	Vertex v1 = 
	{
		{p_point.x + m_thikness,p_point.y + m_thikness,p_point.z},
...
void LineRenderer::addPoints(std::vector<Vector3> p_points)
{
...
void LineRenderer::setPoint(int p_index, Vector3 p_point)
{
	if (p_index > m_points.size() - 1 || p_index < 0)
		return;
...
Vector3 LineRenderer::getPoint(int p_index)
{
	return m_points[p_index];
}

std::vector<Vector3> LineRenderer::getPoints()
{
	return m_points;
}
...
void LineRenderer::setThikness(float p_thikness)
{
	...
	for (int i = 0; i < m_points.size(); i++)
	{
		...
		Vertex v1 =
		{
			{point.x + m_thikness,point.y + m_thikness,point.z},
			{m_color.x,m_color.y,m_color.z,m_color.w}
		};
		Vertex v2 =
		{
